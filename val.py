...
def main():
    ...
    img_ids = glob(os.path.join('inputs', 'validation_real', 'dataset1', 'val', 'images', '*' + config['img_ext']))
    img_ids = [os.path.splitext(os.path.basename(p))[0] for p in img_ids]

    #trainとtest用の画像を分けたいときはここを変更させる。今は、trainの時に使用したvalの画像を用いた評価している。
    ...
    val_dataset = Dataset(
        img_ids=val_img_ids,
        #testの画像を用いたいときはここにtest画像のソースとマスク画像のpathを書く。
